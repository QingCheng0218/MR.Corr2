...
for(irep in 1:nrep){
  ...
  # Simulate Z(Z=1:correlated, Z=0:independent)
  qrate = rbeta(1, 1, 10);
  qq = L*qrate;
  if(qq<1){
    qno = 1;
  ...
  x12gZ0 = x12Z0%*%gammaZ0;
  
  if(b0!=0){
    h2ga = (h2y *( 1 + b0^2))/(b0^2 * (1 - h2y));
    gamma0Z0 = gammaZ0/sqrt(as.numeric(var(x12gZ0)/h2ga));
    x12gZ0 = x12Z0%*%gamma0Z0;
  }
  
  # for the correlated
  uZ1 = x12Z1%*%gammaZ1 + rnorm(n1+ n2);
  x12gZ1 = uZ1; # Remember that this is the exposure part in correlated model.
  
  ubz1 = uZ1%*%rnorm(1);
  x12gZ1 = x12gZ1/sqrt(as.numeric(var(x12gZ1)/0.2));
  ubz1 = ubz1/sqrt(as.numeric(var(ubz1)/0.2));
  
  # keep var(resy) = 1;
  resy = x12gZ1 + uby + rnorm(n1+n2)*as.numeric(sqrt(1-var(uby + x12gZ1)));
  yall = x12gZ0 + resy;
  
  # The direct effects on Z
  # Control the heritability of alpha on Z
  h2yb = var(b0*yall);
  ...
  if(h2z==0){
    ...
    if(Alrate!=1){
      alno = floor(L*Alrate);
      indA = sample(1:L,alno);
      if(length(indA)==1){
        indxAL = block_inf[indA, 1]:block_inf[indA, 2]
        alpha[-indxAL] = 0;
      }else{
        indxAL = NULL;
        for(i in 1:length(indA)){
          tmp = block_inf[indA[i], 1]:block_inf[indA[i], 2]
  ...
  # keep var(resz) = 1
  resz = ubz + ubz1 + rnorm(n1+n2)*as.numeric(sqrt(1-var(ubz + ubz1)));
  ...
  # H2g.res[irep] <- var(b0*x12gZ0)/var(zall);
  
  # Save the true Eta index
  ALgi = rep(1, L);
  ALgi[ind] = 0;
  if(h2z!=0){
    ALgi[indA] = 0;
